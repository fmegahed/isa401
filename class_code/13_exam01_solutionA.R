...
food_recipes = fromJSON("data/food_recipes.json")
food_recipes[38, ]
...
greek_index = food_recipes$cuisine == 'greek'
sum(greek_index)

## Approach 2: table (easiest approach)

table(food_recipes$cuisine)

## Approach 3: filter (probably what I would use if it was not for table)
pacman::p_load(tidyverse)

greek_recipes = filter(food_recipes, cuisine == 'greek')
...
step2_fb = html_elements(step1_fb, 
                         css = 'span.sidearm-roster-player-weight')
