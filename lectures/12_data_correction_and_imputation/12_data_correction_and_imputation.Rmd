...
agent = 
  ...
  pointblank::create_agent(tbl = bike_tbl, actions = act) |>
  # we will populate the rows without doing the checks
  pointblank::col_is_date(columns = datetime) |> 
  ...
  pointblank::col_is_factor(columns = season:weather) |> 
  # we do not really care if they are dbl vs int but I am showing you that
  # if you did care, we can check for that
  pointblank::col_is_numeric(columns = temp:windspeed) |> 
  pointblank::col_is_integer(columns = casual:count) |> 
  pointblank::col_is_character(columns = vars(sources))

# checks are done in the interrogation step
res = pointblank::interrogate(agent = agent, sample_limit = 5000)
res
```
]
...
.font70[
```{r pointblank_consistency, results='hide'}
agent <-
  # create agent starts the table of checks
  pointblank::create_agent(bike_tbl, actions = act) |>
  # check that the columns are between 0 and 100
  pointblank::col_vals_between(columns = vars(temp, atemp, humidity, windspeed), 0, 100) |> 
  # check that the casual column are greater than or equal to 0
  pointblank::col_vals_gte(columns = vars(casual, registered), 0) |> 
  # check that the count column is greater than 0
  pointblank::col_vals_gt(columns = vars(count), 0) |> 
  # check that the hour column is in a sequence of numbers between 0 and 23
  pointblank::col_vals_in_set(columns = vars(hour), set = seq(0, 23, by = 1)) |> 
  # check that there are no columns in the dataset that have NULLs
  pointblank::col_vals_not_null(columns = names(bike_tbl)) |> 
  # check that the count column is the sum of the casual and registered columns
  pointblank::col_vals_expr(dplyr::expr(count == casual + registered) ) |> 
  # check that the sources column has only the values 'AD campaign' and 'ad campaign'
  pointblank::col_vals_expr(expr = dplyr::expr(sources %in% c('AD campaign', 'ad campaign'))) |> 
  # check that the sources column does not have values other than 'google'
  pointblank::col_vals_expr(expr = ~ stringr::str_detect(sources, pattern = 'google'), label = 'non google sources')

res <- pointblank::interrogate(agent, sample_limit = nrow(bike_tbl))

res |> pointblank::export_report(filename = 'report_consistency.html')
```
