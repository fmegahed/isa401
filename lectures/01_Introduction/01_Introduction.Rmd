...
.panel[.panel-name[Fadel in R (No Solution Shown)]
...
.panel[.panel-name[Fadel in Python (No Solution Shown)]

```{python activity1-solution-py, eval=FALSE}
resp = requests.get("https://data.cincinnati-oh.gov/api/views/rvmt-pkmq/rows.csv?accessType=DOWNLOAD")

with open('crashes.csv', 'w') as f:
    f.write(resp.content.decode('utf-8'))

# Read the data
crashes = pd.read_csv("crashes.csv").clean_names()

# Dimensions
num_rows = len(crashes)
num_cols = len(crashes.columns)
```
...
day_of_week_tbl |> 
  ...
  labs(x = 'Count/Freq', y = 'Day of the Week',
       title = 'Crashes by Day in Cincinnati in 2022',
...
daily_crashes_2022= unique_crashes |> 
...
unique_crashes |> 
  ...
  labs(x = 'Month', y = 'Crashes per Month', 
       title = 'Crashes Recorded in Two Cincinnati Neighborhoods in 2022',
       caption = 'Created by: Fadel Megahed | Data source: City of Cincy Open Data Portal (rvmt-pkmq)')
```
...
- Complete [Assignment 01](https://miamioh.instructure.com/courses/202961/quizzes/573222) on Canvas to reinforce your understanding and application of the topics covered today as well as the assigned readings.
