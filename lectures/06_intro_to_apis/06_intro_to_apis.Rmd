...
subtitle: '06: Connecting to APIs in `r icons::icon_style(icons::fontawesome("r-project"), fill = "white")`'
...
# Quick Refresher from Last Week [1]
...
# Quick Refresher from Last Week [2]

## A Note on HTML Element vs Elements for Tables

In office hours, I was asked a question related to the impact of using `[[]]` on the output from `html_table()` when `step2` utilizes `html_element()` vs `html_elements()`. So let us examine the [Miami Football Wikipedia Page](https://en.wikipedia.org/wiki/Miami_RedHawks_football) to: (a) answer this question; and (b) see the impact of `html_element()` vs `html_elements()` when there are multiple tables of the same class.  


## One more Example on Scraping Multiple Webpages

The most popular listings on Netflix are are available at <https://www.imdb.com/search/title/?companies=co0144901>. Write an `r fontawesome::fa("r-project", fill = miamired)` script that will produce a tibble that contains the **following information for the first 300 entries**: (a)title, which you will save in a column titled `title`; (b) year/years of show, which you will save in a column titled `year`; and (c) a 1-2 sentence summary of show, which you save in a column titled `summary`.  
...
# What is an API? <br> (A Web Server Bases Perspective)
...
# Application #4: Deploy a ML Model

<iframe src="https://hf.space/embed/fmegahed/tavr_project/+" frameBorder="0" height="450" width="1000" title="Gradio app" class="container p-0 flex-grow space-iframe"></iframe>
...
**Source:** To access the app outside of the slides, please visit [fmegahed/tavr_project](https://huggingface.co/spaces/fmegahed/tavr_project).
]
...
accu_key = Sys.getenv()[['accu_key']]

base_accu_url = paste0('http://dataservice.accuweather.com/forecasts/v1/daily/5day/340019?apikey=',
                       accu_key)
...
- Explain how APIs will be a huge part of your career as a business analyst nad/or data scientist  
...
- Go over your notes, read through the supplementary material (below), go through the [self-paced tutorial](http://rstudio.fsb.miamioh.edu:3838/megahefm/isa401/webscraping/), and complete assignments [05](https://miamioh.instructure.com/courses/179812/assignments/2218207) & [06](https://miamioh.instructure.com/courses/179812/assignments/2220815) on Canvas. 
...
- Per our [tentative schedule](https://miamioh.instructure.com/courses/179812/pages/schedule), we will have an in-class lab where you will be expected to combine your new web scraping skills with APIs. 
