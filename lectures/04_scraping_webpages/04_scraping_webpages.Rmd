...
subtitle: '04: Scraping Webpages in `r icons::icon_style(icons::fontawesome("r-project"), fill = "white")` and `r icons::icon_style(icons::fontawesome("python"), fill = "white")`'
...
`r emo::ji("check")` Subset data in `r fontawesome::fa(name = 'r-project', fill = miamired)` and `r fontawesome::fa(name = 'python', fill = miamired)`.   

`r emo::ji("check")` Read text-files, binary files (e.g., Excel, SAS, SPSS, Stata, etc), json files, etc. 

`r emo::ji("check")` Export data from `r fontawesome::fa(name = 'r-project', fill = miamired)` and `r fontawesome::fa(name = 'python', fill = miamired)`. 
...
- Scrape a webpage using `r fontawesome::fa("r-project", fill = miamired)` and `r fontawesome::fa("python", fill = miamired)`
...
# Step 2: Use the CSS selector "p.courseblocktitle > strong" to extract course names
course_names = soup.select("p.courseblocktitle > strong")

# Step 3: Remove the HTML Tags (Extractthe text content of each course name)
...
df = pd.DataFrame(
  list( zip(cleaned_course_names, cleaned_course_descriptions) ),
...
> _Over the next 4 minutes, use an `r fa('r-project')` or a `r fa('python')` script file to perform the tasks outlined in the activity panel._
...
- Go over your notes, read through the supplementary material (below) and complete [Assignment 04](https://miamioh.instructure.com/courses/202961/assignments/2556568) on Canvas. 
