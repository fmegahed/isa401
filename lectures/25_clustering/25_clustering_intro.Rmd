...
subtitle: "25: A Short Introduction to Clustering"
...
row.names(penguins_subset) = paste0( str_sub(penguins_subset$species, 1, 1),
                                     1:nrow(penguins_subset) )

penguins_subset = na.omit(penguins_subset) %>% select(-species)
...
Use the k-means algorithm to cluster the following observations. Use $k=2$ and Euclidean distance. **Use [this handout](https://miamioh.instructure.com/courses/179812/files/26097060?module_item_id=4084620) to go through the k-means algorithm implementation (by hand).**
...
**Solution:** Once you complete the handout, you can check your solution by downloading [this file](https://miamioh.instructure.com/files/26097061/download?download_frd=1). 
]
...
Let us use Tableau to implement the k-means clustering implementation on the 60 sample observations from the penguins dataset as shown in Slide 7 of this presentation.
