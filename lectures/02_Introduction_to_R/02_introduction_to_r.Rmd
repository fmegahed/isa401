...
- Utilize the project workflow in `r fa(name = "r-project")` and create your second `r fa(name = "r-project")` script. 
...
- As of `r Sys.Date() %>% format(format = '%b %d, %Y')`, there are 18,491 packages on [CRAN](https://cran.r-project.org/) (not including those on `r fontawesome::fa("github")`).   
...
While we will not specifically talk about doing math in `r fa(name = "r-project")`, the operators below are good to know.
...
# Your Second `r fa(name = "r-project")` Script File
...
- Complete [Assignment 02](https://miamioh.instructure.com/courses/179812/quizzes/501682) on Canvas.  
