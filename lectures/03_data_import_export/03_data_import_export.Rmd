...
# Some students may get SSL-related issues: (Fix below)
# -----------------------------------------------------

# workarounds for SSL issues with pandas
...
wh =req.get('https://www.whitehouse.gov/wp-content/uploads/2022/05/2022.01_WAVES-ACCESS-RECORDS.csv')
wh1 = pd.read_csv(io.StringIO(wh.text))
wh2 = pd.read_csv(io.BytesIO(wh.content))
```
...
```{python json_demo, include = F}
# JSONs:
# ----------
# read_json often does not work from a web URL (so download it or follow approach below)
rep_get = req.get('https://www.govtrack.us/api/v2/role?current=true&role_type=representative&limit=438').json()

rep_get.keys()

rep = pd.DataFrame.from_dict(rep_get['objects'])
rep
```
