...
- Import CSV files into your `r fa('r-project', fill=miamired)` environment based on:
...
Use an appropriate function from the `readxl` package to read the data in the `Incidents` tab (either `read_xlsx()` or `read_xls()`).
...
## Your First Resume-Worthy Artificat: Job Explorer Demo

.font80[
In this demo, we will use the [OEWS Dataset](https://www.bls.gov/oes/tables.htm) from the U.S. Bureau of Labor Statistics (BLS) to create a quick data explorer app using the [querychat](https://cran.r-project.org/web/packages/querychat/index.html) `r fontawesome::fa('box', fill = 'gold')` package.

**Steps:**  

- Go to [Hugging Face](https://huggingface.co/) and create an account.

- Let us download the [May 2024: All data (XLSX)](https://www.bls.gov/oes/special-requests/oesm24all.zip) file from the BLS website.  

- Unzip and read the data into your `r fontawesome::fa('r-project', fill = miamired)` environment using the `readxl` `r fontawesome::fa('box', fill = 'gold')` package.  

- Set up the `querychat` app to explore the data using natural language.

- Run the app and interact with it.  

- Let us leverage the [DockerFile](https://huggingface.co/spaces/fmegahed/querychat_demo/resolve/main/Dockerfile), [data_desc.md](https://huggingface.co/spaces/fmegahed/querychat_demo/resolve/main/data/data_desc.md), and [extra_instructions.md](https://huggingface.co/spaces/fmegahed/querychat_demo/resolve/main/data/extra_instructions.md) files to improve the app and deploy it on Hugging Face Spaces.

- Take **notes** of the steps and process so you can do this for any dataset of interest.
]
...
We will use the [jsonlite](https://cran.r-project.org/web/packages/jsonlite/index.html) `r fa('box', fill = 'gold')` to read an example from [Cincinnati Job Posting on The Muse ](https://www.themuse.com/api/public/jobs?page=1&location=Cincinnati%2C%20OH).
...
```{r themuse, include=FALSE}

url = "https://www.themuse.com/api/public/jobs?page=1&location=Cincinnati%2C%20OH"

# 1) Read JSON into an R list
job_json <- jsonlite::fromJSON(url, simplifyVector = TRUE)

# Quick peek at top-level fields
names(job_json)
# Common ones include: results, page, page_count, items_per_page, total

# 2) Extract the job postings (a list of jobs)
jobs <- job_json$results
length(jobs)

# 3) Turn into a tibble while safely handling nested structures
jobs_clean = jobs |> dplyr::select(-c(categories, tags)) |>  tidyr::unnest()
