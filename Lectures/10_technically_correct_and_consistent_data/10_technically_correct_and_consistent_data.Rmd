...
subtitle: "10: Towards Technically Correct and Consistent Data"
...
- Manipulate characters (string normalization and approximate string matching)
...
# renaming with rename and two-way pipe
iris_tbl %<>% rename(c(sepal_length = sepal_l, sepal_width = sepal_w)) #<<
names(iris_tbl)
```
]
...
# Non-Graded Class Activity: Exploring the Previous Functions
...
> _Over the next 5 minutes, please examine the following:_ 

- Use `pacman::p_load()` to install/load the previous packages. Notes:  
  * `base` R packages are loaded with each R session.
  * `purrr` and `dplyr` are part of the `tidyverse` echo system.

- Run the following code: `iris_tbl = tibble(iris) %>% clean_names()` and examine the 7 approaches highlighted in the previous slide.
]

.panel[.panel-name[Your Solution]
- Please write a script file in RStudio to examine the different approaches.  

- The purpose of this activity is two-fold:  
  * Making sure that you can make these different functions work.  
  * Allowing you to develop a preference for quickly examining different datasets.
  
- As promised, I will **never** force you to follow a specific approach. 
]
]
...
- **Convert to Factor:** 
  * To convert a numeric vector to factors, we will typically use the chain of `as_character() %>% as_factor()`.  
...
- Read the [bike_sharing_data.csv](https://raw.githubusercontent.com/fmegahed/isa401/main/Data/apple.csv) data. The description of all fields (with the exception of the `sources` column) can be found at [Kaggle dataset description](https://www.kaggle.com/c/bike-sharing-demand/data).  
