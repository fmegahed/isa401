...
subtitle: "04: Scraping Webpages in R"
...
.small[Based on the readr package, the following function is appropriate for the purpose of reading the file [patterson_cafe_yelp_reviews.rds](https://github.com/fmegahed/isa401/blob/main/Data/patterson_cafe_yelp_reviews.rds?raw=true)  

`r emo::ji("x")` `read_csv()`

`r emo::ji("x")` `read_fwf()`

`r emo::ji("check")` `read_rds()`  
...
cafe_yelp_reviews_base = readRDS(file = "../../Data/patterson_cafe_yelp_reviews.rds")
...
> _Over the next 4 minutes, use an R script file to perform the tasks outline in the activity panel._
...
- We will build on the previous example and we will scrape all the plane crashes that were recorded in the [plane crash database](http://www.planecrashinfo.com/) between 2012-2021. 
...
If time allows, we will download the [PDFs of the ISA 401 lectures from GitHub](https://github.com/fmegahed/isa401/tree/main/PDFs).
