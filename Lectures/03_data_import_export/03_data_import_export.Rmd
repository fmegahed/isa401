...
`r emo::ji("check")` Describe how and why we are using R in this course?  

`r emo::ji("check")` Understand the syntax, data structures and functions  

`r emo::ji("check")` Utilize the project workflow in R  
...
**Winning the competition involves having as many correct answers as possible AND taking the shortest duration to answer these questions.** The winner `r fontawesome::fa(name = 'trophy', fill = 'gold')` of the competition from each section will receive:  $15 `r fontawesome::fa(name = 'coffee', fill ='green')` gift card. Good luck!!!
...
`r emo::ji("check")` Record information  

`r emo::ji("check")` Analyze data to support reasoning  

`r emo::ji("check")` Communicate ideas to others  

`r emo::ji("check")` Interact with the data
...
.pull-left-2[
```{r cases_viz1, echo=FALSE, fig.height=2.5}
knitr::include_graphics('figures/cases.PNG')
```
]
...
`r emo::ji("check")` coronavirus cases per county   

`r emo::ji("check")` U.S. counties 
]
...
.pull-left-2[
```{r cases_viz2, echo=FALSE, fig.height=2}
knitr::include_graphics('figures/cases.PNG')
```
]

.pull-right-2[
This is `r emo::ji("x")`. **Let us discuss why**. 
]
...
- **While attempting the assignment, you are encouraged to:**  
  * Google (`r fontawesome::fa(name = 'google', fill = miamired)`) any `r fontawesome::fa(name = 'r-project', fill = miamired)` that you need.  
