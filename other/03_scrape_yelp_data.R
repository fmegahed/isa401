...
urls = paste0("https://www.yelp.com/biz/pattersons-cafe-oxford?osq=Restaurants&start=",
              seq(from=0,to = 90, by = 10))

scrape_yelp = function(x){
  read_page = read_html(x)
  
  reviewer = read_page %>% 
   html_nodes("span.fs-block.css-1iikwpv > a.css-1422juy") %>% html_text()
  ...
  review_date = read_page %>% 
    html_nodes("div.arrange-unit__09f24__rqHTg.arrange-unit-fill__09f24__CUubG.border-color--default__09f24__NPAKY > span.css-1e4fdj9") %>% 
